<?php
	class DfoxwGrab{
		...
		public function __construct(){
			// 引入插件
			dfox_wp_load_plugins(array('ParserDom'));
			/**
			 * 检查或创建数据表
			 */
			global $wpdb;
			$this->dfoxw_table = $wpdb->prefix . 'dfoxwgrab';
			$this->checkGrabTable();
		...
		public function createPost($event,$post){
			$args = array();
			// 作者
			if($event['dfoxw_save_author'] == 'custom' && is_user_member_of_blog($event['dfoxw_save_authorid']))
				$args['post_author'] = $event['dfoxw_save_authorid'];
			// 文章状态
			$args['post_status'] = $event['dfoxw_save_poststatus'];

			// 时间
			if($event['dfoxw_save_poststatus'] == 'future'){
				$args['post_status'] = 'publish';
				$args['post_date'] = $event['post_date'];
			}

			$args['post_content'] = $this->getPostContent($post['post_content'],$event);
			if($event['dfoxw_save_style'] == 'reset')
				$args['post_content'] = "<div id='dfoxw_resetwechar'>{$args['post_content']}</div>";

			$args['post_title'] = $post['post_title'];

			$args['post_category'] = $event['dfoxw_save_category'];

			$args['meta_input'] = array();

			if($event['dfoxw_save_meta_author'] != '')
				$args['meta_input'][$event['dfoxw_save_meta_author']] = $post['author'];

			if($event['dfoxw_save_meta_desc'] != '')
				$args['meta_input'][$event['dfoxw_save_meta_desc']] = $post['desc'];

			if($event['dfoxw_save_meta_views'] != '')
				$args['meta_input'][$event['dfoxw_save_meta_views']] = $post['views'];

			if($event['dfoxw_save_meta_likes'] != '')
				$args['meta_input'][$event['dfoxw_save_meta_likes']] = $post['likes'];

			if($event['dfoxw_save_meta_sourceurl'] != '')
				$args['meta_input'][$event['dfoxw_save_meta_sourceurl']] = $post['sourceurl'];

			$args['meta_input']['_dfoxw_type'] = 'wechat';
			...
			$postID = $request;
			if($event['dfoxw_save_cover'] === 'cover'){
				if($post['sourceurl'] != ''){
			...
			}elseif($event['dfoxw_save_cover'] === 'first'){
				...
				// }
			}elseif($event['dfoxw_save_cover'] === 'cover&first'){
				if($post['sourceurl'] != ''){
					$sourceurl = $this->saveRemoteImage($post['sourceurl']);
					if(!$sourceurl){
						set_post_thumbnail($postID,$sourceurl['attach_id']);
		...
		public function delGrab($grabID){
			global $wpdb;
			$sql = "UPDATE {$this->dfoxw_table} SET `status` = '0' WHERE `ID` = %d";
			$request = $wpdb->query($wpdb->prepare($sql,$grabID));
			wp_cache_delete('dfoxw_cache_grablists');
		}
		...
		public function getGrabLists(){
			if(wp_cache_get('dfoxw_cache_grablists'))
				return wp_cache_get('dfoxw_cache_grablists');

			$sql = "SELECT * FROM {$this->dfoxw_table} WHERE `status` != 0 ORDER BY `ID` DESC";
			global $wpdb;
			$request = $wpdb->get_results($sql);
			if($request == null || count($request) == 0)
				return null;

			wp_cache_set('dfoxw_cache_grablists',$request);
			return $request;
		}
		...
		public function getGrabListsHtml(){
			...
			ob_start();
		?>
			<table class="form-table dfox-wp-listtable">
				<thead>
					<tr>
						<th>时间(执行/创建)</th>
						<th class="dfox-wp-thead-min">类型</th>
						<th class="dfox-wp-thead-min">状态</th>
						<th>事件内容</th>
						<th>其他</th>
						<th class="dfox-wp-align-right">操作</th>
					</tr>
				</thead>
				<tbody>
				<?php
				...
				?>
					<tr data-grab-id="<?php esc_attr_e($grab->ID); ?>">
						<td class="dfox-wp-layout-tb">
							<h4>
								<?php
									if($grab->status == 3)
										echo '手动激活';
						...
						<td class="dfox-wp-maskitem">
							<div class="dfox-wp-maskbox">
								<span>图像资源:<?php echo $event['dfoxw_save_image'] == 'save' ? "保存至本地" : "使用远程地址";  ?></span>
								<span>特色图像:
								<?php 
									switch ($event['dfoxw_save_cover']) {
										case 'cover':
											echo "使用封面";
											break;
										case 'first':
								...
								<span>作者ID:<?php echo $event['dfoxw_save_author'] == 'current' ? get_current_user_id() : $event['dfoxw_save_authorid']; ?></span>
								<span>文章状态:
								<?php 
									switch ($event['dfoxw_save_poststatus']) {
										case 'publish':
											echo "已发布";
											break;
										case 'private':
								...
								<span>样式重置:<?php echo $event['dfoxw_save_style'] == 'reset' ? "重置" : "不重置"; ?></span>
								<span>保存分类:<?php echo $this->arrayToString($event['dfoxw_save_category']); ?></span>
								<span class="dfox-wp-line">自定义栏目</span>
								<?php if($event['dfoxw_save_meta_author'] != '') echo "<span>文章作者:{$event['dfoxw_save_meta_author']}</span>"; ?>
								<?php if($event['dfoxw_save_meta_desc'] != '') echo "<span>文章描述:{$event['dfoxw_save_meta_desc']}</span>"; ?>
								<?php if($event['dfoxw_save_meta_views'] != '') echo "<span>文章阅读数:{$event['dfoxw_save_meta_views']}</span>"; ?>
								<?php if($event['dfoxw_save_meta_likes'] != '') echo "<span>文章点赞数:{$event['dfoxw_save_meta_likes']}</span>"; ?>
									<?php if($event['dfoxw_save_meta_sourceurl'] != '') echo "<span>阅读原文:{$event['dfoxw_save_meta_sourceurl']}</span>"; ?>
							</div>
						</td>
						<td class="dfox-wp-tags">
							<?php
								$loadposturls = array();
								foreach ($event['posturls'] as $url) {
									if($this->checkWechatUrl($url['url'])){
										$this->getMinUrl($url['url'],$url['status']);
										if($url['status'] == 0){
											$loadposturls[] = $url['url'];
										}
									}
								}
							?>
						</td>
						<td class="dfox-wp-align-right">
						<form method="post" class="grab-form">
							<?php wp_nonce_field('dfoxw_grab', 'dfoxw_grab_field'); ?>
							<input type="hidden" name="dfoxw_grabid" value="<?php esc_attr_e($grab->ID); ?>">
							<input type="hidden" name="dfoxw_urls" value='<?php echo json_encode($loadposturls); ?>'>
		...
		public function getPostContent($htmlbody,$event){
			...
			foreach ($html_dom->find('img') as $imgtag) {
				...
				if($event['dfoxw_save_image'] == 'save'){
			...
			foreach ($html_dom->find('.video_iframe') as $iframe) {
				...
				if($event['dfoxw_save_video'] == 'auto'){
		...
		public function getPostPlaceElement($html,$tagname,$attr = '',$place = null){
			$html_dom = new \HtmlParser\ParserDom($this->wrapHtmlCode($html));
			$tags = $html_dom->find($tagname);
		...
		public function updateEventUrlStatus($grabID,$url,$status){
			...
			global $wpdb;
			$request = $wpdb->update($this->dfoxw_table,array('event' => maybe_serialize($event)),array('ID' => $grabID),array('%s'),array('%d'));
			if($request === 1){
				wp_cache_delete('dfoxw_cache_grablists');
				return true;
			}
		...
		public function updateGrabStatus($grabID,$status){
			$grab = $this->getGrab($grabID);
			global $wpdb;
			$request = $wpdb->update($this->dfoxw_table,array('status' => $status),array('ID' => $grabID),array('%d'),array('%d'));
			if($request === 1){
				wp_cache_delete('dfoxw_cache_grablists');
				return true;
			}
		...
		public function createGrab($args){
			...
			$event_element = array(
				...
				'dfoxw_save_author',
				'dfoxw_save_authorid',
				'dfoxw_save_poststatus',
				'dfoxw_save_style',
				'dfoxw_save_category',
				'dfoxw_save_meta_author',
				'dfoxw_save_meta_desc',
				'dfoxw_save_meta_views',
				'dfoxw_save_meta_likes',
				'dfoxw_save_meta_sourceurl'
			);
			...
			global $wpdb;
			$request = $wpdb->insert($this->dfoxw_table,array(
		 ...
		 * 验证微信 url 地址是否合法合规
		 * @Author @快叫我韩大人
		 * @param  string  $url  微信 URL 地址
		 * @return boolean       返回是否正确 true 或 false
		 */
		public function checkWechatUrl($url){
			if($url != '' &&
				strpos($url,'weixin') !== false &&
				strpos($url,'qq') !== false &&
		 ...
		 * 验证微信 URL数组,并返回正确的内容
		 * @Author @快叫我韩大人
		 * @param  array  $urls  需要验证的 URL 数组
		 * @return array         返回验证成功的url
		 */
		public function checkWechatUrls($urls){
			$ret = array();
			foreach ($urls as $url) {
				if($this->checkWechatUrl($url)){
					$ret[] = $url;
				}
			}
			return $ret;
		...
		private function checkGrabTable($create = true){
			global $wpdb;
			$wpdb->hide_errors();
			$status = $wpdb->query("SELECT * FROM `{$this->dfoxw_table}`");
			if($status !== false)
				return true;
